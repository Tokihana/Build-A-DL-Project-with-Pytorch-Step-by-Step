...
def create_RepVGGplus_L2pse(deploy=False, use_checkpoint=False, config=None):
    if config is not None:
        return RepVGGplus(num_blocks=[8, 14, 24, 1], num_classes=config.MODEL.NUM_CLASS,
...
def create_RepVGGplus_by_name(name, deploy=False, use_checkpoint=False, config=None):
    if 'plus' in name:
        return repvggplus_func_dict[name](deploy=deploy, use_checkpoint=use_checkpoint, config=config)
    else:
